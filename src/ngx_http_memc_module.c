...
static ngx_int_t ngx_http_memc_create_request(ngx_http_request_t *r);
static ngx_int_t ngx_http_memc_reinit_request(ngx_http_request_t *r);
static ngx_int_t ngx_http_memc_process_header(ngx_http_request_t *r);
static ngx_int_t ngx_http_memc_filter_init(void *data);
static ngx_int_t ngx_http_memc_filter(void *data, ssize_t bytes);
...
#define NGX_HTTP_MEMCACHED_END   (sizeof(ngx_http_memc_end) - 1)
static u_char  ngx_http_memc_end[] = CRLF "END" CRLF;
